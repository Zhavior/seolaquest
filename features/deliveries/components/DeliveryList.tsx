...
export function DeliveryList({ deliveries: initialDeliveries, hasMore: initialHasMore }: DeliveryListResult) {
  ...
  if (deliveries.length === 0) {
    return (
      <div className="border-4 border-outline bg-card p-8 text-center shadow-brutal-lg">
        <div className="mx-auto mb-4 inline-flex items-center gap-2 border-2 border-outline bg-accent px-3 py-1 text-xs font-black uppercase shadow-brutal-sm">
        ...
        <p className="mx-auto mt-3 max-w-xl text-sm font-bold text-ink/70">
          This page reports backend delivery state. It does not claim a CRM accepted anything until the worker records delivery.
        </p>
        <Link
          href="/app"
          className="mt-6 inline-flex border-4 border-outline bg-accent px-6 py-3 text-sm font-black uppercase shadow-brutal hover:bg-yellow-300 active:translate-y-0.5"
        >
  ...
  return (
    ...
    >
      ...
      <motion.div variants={item} className="relative flex items-center border-4 border-outline bg-card shadow-brutal">
        ...
        <input
          ...
          className="w-full bg-transparent py-3 pl-10 pr-10 text-xs sm:text-sm font-black uppercase text-ink placeholder:text-ink/40 focus:outline-none"
        />
        {searchQuery && (
          <button
            type="button"
        ...
        {filteredDeliveries.map((delivery) => (
          <motion.div key={delivery.id} variants={item}>
            <Link
              ...
              className="group grid gap-4 border-4 border-outline bg-card p-5 shadow-brutal-lg transition-all hover:-translate-x-0.5 hover:-translate-y-0.5 hover:shadow-brutal-lg md:grid-cols-[minmax(0,1fr)_auto]"
            >
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-3">
                  <DeliveryStatusBadge status={delivery.status} />
                  <span className="border-2 border-outline bg-accent px-2.5 py-0.5 font-mono text-xs font-black shadow-brutal-sm">
                    DELIVERY {shortDeliveryId(delivery.id)}
                  </span>
                </div>
                <p className="mt-3 text-sm font-black uppercase text-ink">{delivery.statusMessage}</p>
                <p className="mt-2 text-xs font-bold uppercase text-ink/60">
                  Updated: {formatDeliveryTime(delivery.updatedAt)}
                </p>
              </div>

              <div className="flex items-center justify-between gap-5 border-t-3 border-outline pt-3 md:block md:min-w-36 md:border-l-3 md:border-t-0 md:pl-5 md:pt-0">
                <span className="text-[10px] font-black uppercase tracking-wider text-ink/60">Delivery Attempts</span>
                <div className="mt-1 border-2 border-outline bg-highlight px-3 py-1.5 font-black text-sm text-on-accent shadow-brutal-sm">
      ...
      {hasMore && (
        <motion.div variants={item} className="text-center pt-2">
          <button
            onClick={loadMore}
            disabled={isPending}
            className="border-4 border-outline bg-accent px-6 py-3 font-black uppercase shadow-brutal hover:-translate-y-0.5 active:translate-y-0 active:shadow-none disabled:opacity-50 transition-all hover:bg-yellow-300 text-on-accent"
          >
      ...
      {!hasMore && deliveries.length > 0 && (
        <motion.p variants={item} className="border-3 border-outline bg-card p-3 text-center text-xs font-black uppercase shadow-brutal-sm text-ink">
          End of deliveries.
        </motion.p>
      )}
    </motion.div>

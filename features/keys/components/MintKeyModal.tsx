...
export default function MintKeyModal({ isOpen, onClose }: MintKeyModalProps) {
  return (
    <AccessibleDialog
      ...
      panelClassName="relative w-full max-w-lg border-4 border-outline bg-card p-6 shadow-brutal-lg"
    >
        <button onClick={onClose} className="absolute right-4 top-4 flex h-11 w-11 items-center justify-center border-2 border-outline bg-card" aria-label="Close key dialog">
          <X className="h-5 w-5" />
        </button>
        ...
        <button onClick={onClose} className="mt-6 min-h-11 border-4 border-outline bg-accent px-5 py-3 font-black uppercase shadow-brutal">
          Close
        </button>
    </AccessibleDialog>
  )

    ...
    >
      <div className="mx-auto max-w-7xl grid lg:grid-cols-[1fr_420px] gap-12">
        ...
        <div>
          ...
          <div className="border-4 border-outline shadow-brutal-lg overflow-hidden">
            <table className="w-full" role="table" aria-label="Guild leaderboard">
              <thead>
                <tr className="border-b-4 border-outline" style={{ background: '#FFE600' }}>
              ...
              <tbody>
                {GUILD_ROSTER.map(({ name, class: cls, leads, badge }, i) => (
                  <tr
                    key={name}
                    className="border-b-2 border-outline last:border-b-0 hover:bg-black hover:text-white transition-colors group"
        ...
        <div className="flex flex-col gap-5">
          ...
          {TESTIMONIALS.map(({ quote, author, class: cls }) => (
            <blockquote
              key={author}
              className="border-4 border-outline p-5 shadow-brutal"
              style={{ background: '#f4ebd8' }}
            >
              <p className="font-bold text-sm sm:text-base leading-relaxed mb-3">&ldquo;{quote}&rdquo;</p>
              <footer className="flex items-center gap-2">
                <span
                  className="border-2 border-outline px-2 py-0.5 text-[9px] font-black uppercase tracking-widest"
                  style={{ background: '#FFE600' }}
                >
                  {cls}
                </span>

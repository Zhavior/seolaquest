    ...
    >
      <div>
        ...
        <div className={questSurface({ tone: 'parchment', shadow: 'md', className: 'relative mb-6 overflow-hidden p-5' })}>
          ...
          <h3 className="text-xl md:text-2xl font-black text-ink uppercase break-word-safe relative z-10 leading-snug">
            &quot;{deadliestArtifact}&quot;
          </h3>

          <div className="mt-4 flex justify-between items-end relative z-10 pt-4 border-t-2 border-outline/20">
            <div className="flex min-w-0 flex-col">
              <span className="text-xs uppercase font-extrabold text-ink-muted">Contacted lead records</span>
              <span className="text-3xl md:text-4xl font-black text-[#FF5722] drop-shadow-brutal-sm">
        ...
        <div className="space-y-3 mb-6">
          ...
          <div className={questSurface({ tone: 'parchment', border: 2, shadow: 'sm', className: 'space-y-2 p-4' })}>
            {channels.length === 0 && <p className="text-sm font-bold text-ink-muted">No stored channel distribution yet.</p>}
            {channels.map((chan, idx) => (
              <div key={idx} className="space-y-1">
                ...
                <div className="w-full bg-card border-2 border-outline h-4 overflow-hidden">
                  <motion.div
                    initial={{ width: 0 }}
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className={questSurface({ tone: 'parchment', shadow: 'sm', className: 'p-4' })}>
          <div className="flex items-center gap-2 mb-1">
            ...
            <p className="text-xs font-black text-ink-muted uppercase">Critical Hit Rate</p>
          </div>
          <p className="text-xl font-black text-ink">Not measured</p>
          <p className="text-[10px] font-bold text-ink-muted uppercase mt-0.5">No conversion event model</p>
        </div>

        <div className={questSurface({ tone: 'parchment', shadow: 'sm', className: 'p-4' })}>
          <div className="flex items-center gap-2 mb-1">
            <Clock aria-hidden="true" className="w-4 h-4 text-blue-600" />
            <p className="text-xs font-black text-ink-muted uppercase">Scout Speed</p>
          </div>
          <p className="text-2xl font-black text-ink truncate">{scoutSpeed}</p>
          <p className="text-[10px] font-bold text-ink-muted uppercase mt-0.5">Only shown when measured</p>
        </div>
      </div>
    </motion.section>
  )

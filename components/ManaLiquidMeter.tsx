...
export default function ManaLiquidMeter({
  ...
  return (
    <div
      className={`relative overflow-hidden border-4 border-outline p-3 md:p-4 flex flex-col md:flex-row items-center justify-between gap-4 shadow-brutal-lg transition-colors duration-500 ${
        isCritical
    ...
    >
      ...
      {isCritical && (
        <div className="absolute top-0 left-0 right-0 bg-[#FF0055] text-white font-black text-[11px] uppercase tracking-widest text-center py-0.5 z-20 flex items-center justify-center gap-2 border-b-2 border-outline animate-bounce">
      ...
      <div className={`flex items-center gap-3 relative z-10 ${isCritical ? 'mt-4 md:mt-0' : ''}`}>
        <div
          className={`p-2.5 border-2 border-outline shadow-[2px_2px_0_0_#fff] ${
            isCritical ? 'bg-[#FF0055] text-white' : 'bg-info text-on-accent'
          }`}
        >
          <FlaskConical className="w-6 h-6 stroke-[3px]" />
        </div>
        <div>
          <div className="flex items-center gap-2 font-black text-xs uppercase tracking-wider text-ink-muted">
            <span>Mana Tank</span>
            {isCritical && (
          ...
          <div className="font-black text-2xl tracking-tight flex items-baseline gap-1">
            ...
            <span className="text-sm text-ink-muted font-mono">/ {maxMana} MP</span>
          </div>
        </div>
      </div>
      ...
      <div className={`w-full md:w-72 relative z-10 ${isCritical ? 'mt-1' : ''}`}>
        <div className="relative h-9 w-full bg-zinc-900 border-4 border-white overflow-hidden shadow-brutal-sm">
            ...
            {isCritical &&
              sparks.map((spark) => (
                <motion.div
                  ...
                  style={{ left: `${spark.x}%` }}
                  className="absolute bottom-0 w-2 h-2 bg-accent rounded-full shadow-[0_0_6px_#FFE600] pointer-events-none z-20"
                />
              ))}
          </AnimatePresence>
      ...
      <div className="relative z-10 w-full md:w-auto">
        <motion.button
          ...
          className={`w-full md:w-auto font-black text-sm uppercase px-5 py-2.5 border-4 border-outline shadow-brutal flex items-center justify-center gap-2 cursor-pointer transition-colors ${
            isCritical
              ? 'bg-accent text-on-accent hover:bg-yellow-300 animate-bounce'

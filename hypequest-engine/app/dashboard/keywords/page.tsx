...
import { Shield, Search, Plus, Trash2, Zap, Radio, CheckCircle } from 'lucide-react'
...
const HERO_CLASSES = [
  { name: 'Warrior 🥷', desc: 'Aggressive High-Intent Leads' },
  { name: 'Mage 🧙‍♂️', desc: 'SaaS & Software Mentions' },
  { name: 'Knight 🦸‍♂️', desc: 'Freelance & Contract Requests' },
  { name: 'Slayer 🧛‍♂️', desc: 'Competitor Complaints & Swaps' }
]

export default function KeywordsPage() {
  const [keywords, setKeywords] = useState<Keyword[]>([])
  const [newPhrase, setNewPhrase] = useState('')
  const [selectedHero, setSelectedHero] = useState('Warrior 🥷')
  const [selectedPlatform, setSelectedPlatform] = useState('Reddit & Twitter')
    ...
    <div className="min-h-screen bg-[#F4F0EA] text-black p-4 md:p-8 space-y-8 max-w-6xl mx-auto">
      ...
      <div className="bg-[#FFE600] border-4 border-black p-6 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] -rotate-1 inline-block">
        <h1 className="text-3xl md:text-4xl font-black uppercase tracking-wider flex items-center gap-3">
          <Search className="w-8 h-8 stroke-[3px]" /> Keyword Hero Roster
        </h1>
        <p className="font-bold text-sm mt-1">Deploy 8-Bit Hero Agents to monitor target keywords across social networks.</p>
      </div>
      ...
      <form onSubmit={handleAddKeyword} className="bg-white border-4 border-black p-6 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] space-y-6">
        <h2 className="text-xl font-black uppercase flex items-center gap-2">
          <Plus className="w-6 h-6 stroke-[3px]" /> Assign New Keyword Quest
        </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {/* HERO CLASS SELECTOR */}
          <div>
            <label className="block text-xs font-black uppercase mb-2">Assigned Hero Agent Class</label>
            <select
              value={selectedHero}
              onChange={(e) => setSelectedHero(e.target.value)}
              className="w-full bg-[#F4F0EA] border-3 border-black p-3 font-black text-sm focus:outline-none shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]"
            >
              {HERO_CLASSES.map((h) => (
                <option key={h.name} value={h.name}>{h.name} ({h.desc})</option>
              ))}
            </select>
          </div>
        </div>
        ...
        <motion.button
          ...
          type="submit"
          disabled={isAdding}
          className="bg-[#A3E635] hover:bg-lime-400 font-black text-sm uppercase py-3 px-6 border-3 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex items-center gap-2"
        >
          <Zap className="w-5 h-5 stroke-[2.5px]" /> Deploy Hero Agent
        </motion.button>
      </form>
            ...
            >
              ...
              <div className="flex items-center gap-4 shrink-0">
                ...
                <span className="bg-green-200 text-green-900 border border-green-900 font-black text-xs px-3 py-1.5 flex items-center gap-1">
                  <Radio className="w-4 h-4 animate-pulse text-green-700" /> Scanning
                </span>
              </div>
            </div>
          ))}

...
export default function ApiKeysPage() {
  return (
    <div className="mx-auto min-h-[70vh] max-w-5xl space-y-6 p-4 md:p-8">
      <section className="relative overflow-hidden border-4 border-outline bg-card p-7 shadow-brutal-lg sm:p-10">
        <KeyRound className="absolute -right-8 -top-8 h-48 w-48 text-[#A855F7] opacity-10" />
        <div className="relative max-w-3xl space-y-4">
          <span className="inline-flex items-center gap-2 border-2 border-outline bg-accent-2 px-3 py-1 text-xs font-black uppercase text-on-accent shadow-brutal-sm">
      ...
      <section className="border-4 border-outline bg-highlight p-6 shadow-brutal-lg">
      ...
      <div className="flex flex-wrap gap-3 text-xs font-black uppercase">
        <Link href="/api-terms" className="border-3 border-outline bg-[#A855F7] px-5 py-3 text-on-accent shadow-brutal">Read API status</Link>
        <Link href="/specs" className="border-3 border-outline bg-card px-5 py-3 shadow-brutal">Architecture gates</Link>
      </div>
    </div>
  )
}
